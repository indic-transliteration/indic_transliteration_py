...
def get_approx_deduplicating_key(text, encodingScheme = sanscript.DEVANAGARI):
  """
  ...
  """
  if encodingScheme == sanscript.DEVANAGARI:
    key = text
    ...
    key = regex.sub("([क-हक़-य़], key)्\\1+", "$1", key)
    ...
    logging.warning("got script {} for '{}'".format(encodingScheme, text))
